...
import type { emileMoselyPost } from '@/types'
 ...
 */
async function addMetaToPost(post: CollectionEntry<'emileMoselly'>): Promise<emileMoselyPost> {
 ...
 */
export async function checkPostSlugDuplication(posts: CollectionEntry<'emileMoselly'>[]): Promise<string[]> {
...
async function _getPosts(lang?: Language) {
  ...
  const filteredPosts = await getCollection(
    'emileMoselly',
    ({ data }: CollectionEntry<'emileMoselly'>) => {
 ...
 */
async function _getPinnedEmileMosellyPosts(lang?: Language) {
  const posts = await getPosts(lang)
  return posts
...
export const getPinnedEmileMosellyPosts = memoize(_getPinnedEmileMosellyPosts)
 ...
 */
async function _getEmileMosellyPostsByYear(lang?: Language): Promise<Map<number, emileMoselyPost[]>> {
  const posts = await getRegularPosts(lang)
  const yearMap = new Map<number, emileMoselyPost[]>()

  posts.forEach((post: emileMoselyPost) => {
...
export const getEmileMosellyPostsByYear = memoize(_getEmileMosellyPostsByYear)
...
async function _getPostsGroupByTags(lang?: Language) {
  const posts = await getPosts(lang)
  const tagMap = new Map<string, emileMoselyPost[]>()
